...
class Sharding: CliktCommand("sharding") {
  ...
  override fun run() {
    if (httpPort != 0) {
      ...
      runBlocking {
        val result = runLoadTest(kvasAddress, connectionCount, keyCount, method)
        println("""
          |------------------------------------------------------------
          |$result
        """.trimMargin())
...
fun main(args: Array<String>): Unit {
  Main().subcommands(Sharding()).main(args)
}
  ...
  routing {
    route("/") {
      get("{master?}{connections?}{keys?}{method?}") {
        ...
        val results = runLoadTest(masterIP, connectionCount, keyCount, method)
        call.respondText("""
...
suspend fun runLoadTest(masterAddress: String, connectionCount: Int, keyCount: Int, method: String): String {
  ...
  return runTest(keyCount, connectionCount) { backendNum ->
    KvasLoadTestBackend(shardNumber = sharding::shardNumber, shardStubFactory = sharding::getStub, connectionNum = backendNum)
  }
}